...
private:  
  ...
  struct ME_Feature
  {
    enum { MAX_LABEL_TYPES = 511 };  // ks20080728: max 511 labels
    ...
    ME_Feature(const int l, const int f) : _body((f << 9) + l) {
      assert(l >= 0 && l <= MAX_LABEL_TYPES);
      assert(f >= 0 && f <= 0x7fffff); // max 8.3M features
    };
    int label() const { return _body & 0x1ff; }
    int feature() const { return _body >> 9; }
